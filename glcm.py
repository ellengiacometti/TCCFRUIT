...
def GLCM(src):
    ...
    ax1.axis('off')
    ax1.imshow(BoundingBox, cmap=plt.cm.gray)
    ax1.set_title(' BoundingBox IMAGE')
    ax2.axis('off')
    ax2.imshow(s_Result, cmap=plt.cm.gray)
    ax2.set_title('s_Result IMAGE')
    ax3.axis('off')
    ax3.imshow(s_Closing, cmap=plt.cm.gray)
    ax3.set_title('s_Closing  IMAGE')
    plt.show()

    glcm = greycomatrix(BoundingBox, [2], [0], levels=None, symmetric=True, normed=True)[1:, 1:, :, :]
...
if __name__ == '__main__':
    ...
    Atributos = []
    Atributos.append(GLCM(imageDir_val))
