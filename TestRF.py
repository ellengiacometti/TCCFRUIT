...
### PRINTANDO INFO DOS DATASETS
    ...
    print("\n~~~ RF -  CLASSIFICADOR LISO X RUGOSO ~~~")
    clf_RFLR = load('ModelRF_LR.joblib')
    print("[STATUS] Predicting TRAIN DataBase..")
    predictionTrain = clf_RFLR.predict(FeaturesTrain)
    print("Accuracy for RF on TRAINED data: ", accuracy_score(TextureLabelTrain, predictionTrain))
    print("Confusion Matrix: ", confusion_matrix(TextureLabelTrain, predictionTrain))

    print("[STATUS] Predicting TEST DataBase..")
    predictionTest = clf_RFLR.predict(FeaturesTest)
    print("Accuracy for RF on TEST data: ", accuracy_score(TextureLabelTest, predictionTest))
    print("Confusion Matrix: ", confusion_matrix(TextureLabelTest, predictionTest))

    """CLASSIFICADOR COM DEFEITO X SEM DEFEITO"""

    print("\n~~~ RF - CLASSIFICADOR COM DEFEITO X SEM DEFEITO ~~~")
    clf_RFCS = load('ModelRF_CS.joblib')
    print("[STATUS] Predicting Trained DataBase..")
    predictionTrain = clf_RFCS.predict(FeaturesTrain)
    print("Accuracy for RF on TRAINED data: ", accuracy_score(ColorLabelTrain, predictionTrain))
    print("Confusion Matrix: ", confusion_matrix(ColorLabelTrain, predictionTrain))

    print("[STATUS] Predicting TEST DataBase..")
    predictionTest = clf_RFCS.predict(FeaturesTest)
    print("Accuracy for RF on TEST data: ", accuracy_score(ColorLabelTest, predictionTest))
